...
class AssessmentPIDAdult extends Component
{
    public $searchData;
    public $genders;
    public $learnAbout;
    ...
    protected $assPIDAdultRepository;

    public function boot(UserRepository $userRepository, DepartmentRepository $departmentRepository, AppointmentRepository $appointmentRepository, EventCalendarRepository $eventCalendarRepository, AssessmentPIDAdultRepository $assPIDAdultRepository)
    {
    ...
    public function mount($searchData = null)
    {
        try {
            ...
            $categoryId = 2; // 2 = Adult

            $this->departments = $this->departmentRepository->getAllDepartment();
            $this->all_user = $this->userRepository->getAllUser();
            $this->questions = $this->assPIDAdultRepository->getQuestionCollectionAccordingSubCategory($categoryId);
    ...
    public function submit()
    {
        try {
            ...
            // dd($this->formData);
            Session::flash('alert', ['type' => 'success', 'title' => 'Success! ', 'message' => 'Data save successfully!']);
            return redirect()->route('assessment-pid-adult.show', $appointmentId);
    ...
    {
        $data = [
            'title' => "The Personality Inventory for DSM-5 (PID-5) Adult",
            'gender' => $this->genders,
