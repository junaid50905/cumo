                            ...
                            <div class="field__data mt-2">
                                <table class="table table-hover">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td><a href="{{ route('student.profile', $student->appointment_id) }}">({{ $student->student_id }})
                                                    {{ $student->name }}</a></td>
                                            <td>{{ date('d-M-Y', strtotime($student->appointment_date)) }}</td>
                                            ...
                                            <td class="d-flex align-items-center justify-content-end">
                                                <a href="{{ route('pre-admission-income.payment',   ['id' => $student->appointment_id]) }}" wire:navigate
                                                    rel="noopener noreferrer"
                                                    class="btn btn-sm btn-primary btn-rounded waves-effect waves-light mb-2 me-1"
                                                    title="Payment">
                                                    <i class="fas fa-credit-card"></i>
                                                </a>
                                                <a href="{{ route('pre-admission-income-invoice.invoice', $student->appointment_id) }}" wire:navigate
                                                    rel="noopener noreferrer"
                                                    class="btn btn-sm btn-primary btn-rounded waves-effect waves-light mb-2 me-1"
                                                    title="Invoice">
                                                    <i class="fas fa-file-invoice"></i>
                                                </a>
                                                <a href="{{ route('pre-admission-income.edit', $student->id) }}"
                                                    rel="noopener noreferrer"
