                                ...
                                @foreach($collections as $item)
                                    <tr>
                                        ...
                                        <td class="d-flex align-items-center justify-content-end">
                                            <a href="{{ route('assessment-tools.search', ['tool_id' => $item['tool_id'], 'search_id' => $item['appointment_id'], ]) }}"
                                                rel="noopener noreferrer"
                                                class="btn btn-sm btn-primary btn-rounded waves-effect waves-light mb-2 me-1"
                                                title="Setup">
                                                <i class="mdi mdi-cog"></i>
                                            </a>
                                            <a href="{{ route('assessment-tools.show', ['assessment_tool' => $item['tool_id'], 'tool_id' => $item['tool_id'], 'appointmentId' => $item['appointment_id'], 'toolTitle' => $item['tool_title']]) }}"
                                                rel="noopener noreferrer"
                                                class="btn btn-sm btn-primary btn-rounded waves-effect waves-light mb-2 me-1"
                                                title="View">
